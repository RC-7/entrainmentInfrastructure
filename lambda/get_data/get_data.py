...
def getData(filter_expression, projection_attributes):
    response = EXPERIMEN_TABLE.scan(FilterExpression = eval(filter_expression), ProjectionExpression = projection_attributes)

    data_count = response['Count']
    data = response['Items']

    while 'LastEvaluatedKey' in response:
        response = EXPERIMEN_TABLE.query(FilterExpression = eval(filter_expression), ProjectionExpression = projection_attributes)
    ...
    if data_count > 0:
        ...
        return ['No results found for querry', '404']
...
def lambda_handler(event, context):
    ...
    header_value = requestValues =  json.loads(event['headers'])
    requestValues =  json.loads(event['body'])
    projection_attributes = requestValues['projectionAttributes']
    filter_expression = header_lookup[header_value](requestValues)
