...
def stft_test(eeg_data, electrodes_to_plot, np_slice_indexes, save=False, filename=None, plot_averaged=False):
    ...
    for i in electrodes_to_plot:
        ...
        mean_data_overall = np.mean(data_to_plot)
        std_data_overall = np.std(data_to_plot)
        ten_pec = mean_data_overall + std_data_overall
        f = []
        Zxx = []
        while index_test + SAMPLING_SPEED <= len(data_to_plot):
            index = np.index_exp[index_test:index_test+SAMPLING_SPEED - 1]
            sample_slice = data_to_plot[index]
            index_test += SAMPLING_SPEED
            if np.mean(sample_slice) > ten_pec:
                continue
            samples_per_ft = 100
            overlap = 10
            f, t, Zxx_local = signal.stft(x=sample_slice, fs=SAMPLING_SPEED, nperseg=samples_per_ft, noverlap=overlap, nfft=512)
            # print(len(Zxx_local))
            # print(len(Zxx_local[0]))
            Zxx_local = np.mean(Zxx_local, axis=1)
            Zxx.append(Zxx_local)
            # if Zxx == []:
            #     Zxx = Zxx_local
            # else:
            #     print(len(Zxx))
            #     if len(Zxx) == 257:
            #         Zxx = np.stack((Zxx, Zxx_local), axis=1)
            #     # Zxx = np.append(Zxx, Zxx_local[:, None])
            #     else:
            #         Zxx = np.append(Zxx, Zxx_local, axis=1)
            #     # Zxx.append(Zxx_local)
            #     print(len(Zxx[0]))
...
def main():
    ...
    # index_dict = {}
    # # raw_data.pick([ch_names[n] for n in range(0, 3)])
    stft_test(cropped_data, electrodes_to_plot, index_dict, save=True, filename='Jasp_test_2.png',
              plot_averaged=True)

    # stft_by_region(cropped_data, electrodes_to_plot, index_dict, save=True, filename='a_noDigFilter.png',
    #                plot_averaged=True)
